...
Config.DiagnosticsEnabled = false -- Set to false to disable debug messages / BIG RED ZONES
Config.ContainerRespawnMinutes = 10 -- The time in MINS a player must wait before they can search the same trash again.

Config.StrictEntityValidation = true -- Set to false to disable the advanced cheater check. [WaveShield does not like this..]

Config.RestrictToJobs = false -- Set to 'jobname' to lock the trash functionality to a job. [Default: false - anyone can search the trash] -- Config.JobLocked = {'job1', 'job2', 'job3'}
Config.ContainerStorageEnabled = true -- Set to false to disable the stashes

-- [ NEW ] -- 
Config.WipeContainerStorageOnRestart = false -- Set to true if you do not want the dumpster storage to be persistent between restarts

--------------------------------

Config.ConcealmentEnabled = true -- Set to false to disable the hide in dumpster feature
Config.ExitConcealmentZOffset = 0.5  -- Added height value when leaving dumpster (only for servers that have an issue with players getting stuck or falling through floor on leaving dumpster)

Config.ConcealmentRustleAudio = true -- NEW!! - Set to false to disable the sound when hiding in a dumpster (added in v2 to easily find a player hiding in a dumpster)

Config.UseProgressBars = false -- Set to true to enable progress bars (editable in envi-bridge/utils/client.lua) -- work in progress - MAY BE BUGGY - suggested to use false for now - (may be not fully working for all progress bar types, please keep false if you have any issues)

Config.MishapSettings = {
    Enabled = true, -- Set to false to disable the fail
    VerminEventsEnabled = true, -- Set to false to disable RAT / RACOON events
    SharpsEventsEnabled = true, -- Set to false to disable NEEDLE events
    MishapChance = 35, -- The probability, as a percentage, that a player will fail when attempting to search the trash. A higher number means a greater chance of failure.
    SharpsChance = 30, -- The chance, as a percentage, of encountering dirty needles when a failure occurs during trash pickup. This only applies if needle events are enabled.
    SharpsDurationSeconds = 90, -- The time, in seconds, that the dirty needles effect will last. This only applies if needle events are enabled.
    VerminChance = 80, -- The chance, as a percentage, of encountering a rat OR racoon when a failure occurs during trash pickup. This only applies if rat events are enabled and is calculated after the dirty needles chance.
    SharpsHealthLoss = 20, -- The amount of health a player will lose if they are affected by the dirty needles event.
    GenericHealthLoss = 10, -- The of health a player will lose on a generic fail event, where no specific event like dirty needles or a rat is triggered.
    VerminHealthLoss = 5, -- The amount of health a player will lose if they are affected by the rat event. This is a fixed value and only applies if rat events are enabled.
    CritterChance = 70, -- NEW - The chance, as a percentage, of encountering a racoon instead of a rat. This only applies if rat events are enabled.
    CritterHealthLoss = 25, -- NEW - The fixed amount of health a player will lose if they are affected by the racoon event.
}

--------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
    -- Aggressive Ped Settings --

Config.HostileVagrantsEnabled = true -- Set to false to disable hostile hobo events
Config.HostileVagrantRange = 25 -- The distance, in meters, that a player must be from a hobo before they will become hostile. This only applies if hostile hobo events are enabled.
                                  -- BEWARE: This distance is DOUBLED when searching a model from 'otherSearchables' (Homeless Props)

Config.HostileVagrantModels = {  -- Hobo Models
    'a_m_m_tramp_01',
    'a_m_m_trampbeac_01',
    'A_M_M_Hillbilly_02',
    'A_M_M_RurMeth_01',
...
Config.LootDrawSettings = { -- Configuration for the random item selection
    minDraws = 1, -- Number of items to select randomly (DEFAULT: random between 1 and 5)
    maxDraws = 4, -- Number of items to select randomly (DEFAULT: random between 1 and 5)
}
-- NEW! -- Added Rarity (number 1-100) to each item, the higher the number, the rarer the item
Config.ShorelineBinLoot = {
...
Config.ShorelineBinPrizeChance = 5 -- The chance, as a percentage, of finding an extra rare item!
...
Config.SkipPrizeChance = 10 -- The chance, as a percentage, of finding an extra rare item!

--------------------------------------------------------

Config.StreetBinLoot = {
...
Config.StreetBinPrizeChance = 15 -- The chance, as a percentage, of finding an extra rare item!
...
Config.EncampmentPrizeChance = 2 -- The chance, as a percentage, of finding an extra rare item!

----------------------------------------------------------------

Config.RefuseSackLoot = {
...
Config.RefuseSackPrizeChance = 15 -- The chance, as a percentage, of finding an extra rare item!
--------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
    -- Storage Settings --

Config.ShorelineBinSlots = 3 -- The number of slots in the beach can storage
Config.ShorelineBinWeight = 10000 -- The maximum weight of the beach can storage

Config.SkipSlots = 12 -- The number of slots in the dumpster storage
Config.SkipWeight = 50000 -- The maximum weight of the dumpster storage

Config.StreetBinSlots = 4 -- The number of slots in the garbage can storage
Config.StreetBinWeight = 12000 -- The maximum weight of the garbage can storage

Config.EncampmentSlots = 5 -- The number of slots in the other storage
Config.EncampmentWeight = 15000 -- The maximum weight of the other storage

--------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
    -- Exclusive Item Zones --
-- IF YOU ARE IN THESE AREAS, YOU WILL SOMETIMES FIND THESE ITEMS INSTEAD

Config.SignatureLootZones = {
...
BLScav_EncampmentModels = {
    "prop_skid_tent_01",
    "prop_skid_tent_01b",
    "prop_skid_tent_03",
}

BLScav_RefuseSackModels = {
    'prop_rub_binbag_01b',
    'prop_rub_binbag_04',
    'prop_rub_binbag_06',
...
BLScav_OperatorProps = {   -- NEW! You can now add your own searchables for any prop!
    [1] = {
        label = 'Steal Luggage',
        models = {
            "prop_suitcase_01",
...
--------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
    -- Animation Settings --
-- Fail Animations --
Config.VerminMishapAnim = { dict = 'misscarsteal2_bin', anim = 'trev_sink_exit' }
Config.SharpsMishapAnim = { dict = 'misscarsteal2_bin', anim = 'trev_sink_exit' }
Config.GenericMishapAnim = { dict = 'move_p_m_two_idles@generic', anim = 'fidget_sniff_fingers'} 

-- Search Animations --
BLScav_ShorelineBinAnims = {
    { dict = 'anim@gangops@van@drive_grab@', anim = 'grab_drive' },
    { dict = 'amb@code_human_in_car_mp_actions@arse_pick@std@ps@base', anim = 'enter' },
    { dict = 'rcmepsilonism8', anim = 'bag_handler_grab_walk_left' },
    { dict = 'anim@scripted@player@freemode@gen_grab@heeled@', anim = 'low_multi' },
...
Config.ExitConcealmentKey = 73 -- The key to get out of the dumpster while hiding inside. (Default: X)
