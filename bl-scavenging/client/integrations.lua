...
function ApplySharpsInjury()
    if Framework.HasItem("hobo_gloves", 1) then
        Framework.Notify(Config.Lang['dirty_needles_stopped'], "success", 5000)
        return
    end
    BLScav_Sedated = true
    local healthLoss = Config.MishapSettings.SharpsHealthLoss 
    if not healthLoss then
        TraceLog("Error: healthLoss is not defined in Config")
        return
    end
    Framework.LoadAnimDict(Config.SharpsMishapAnim.dict)
    TaskPlayAnim(cache.ped, Config.SharpsMishapAnim.dict, Config.SharpsMishapAnim.anim, 8.0, 8.0, -1, 49, 0, false, false, false)
    local animTime = GetAnimDuration(Config.SharpsMishapAnim.dict, Config.SharpsMishapAnim.anim) * 1000
    ...
    Wait(Config.MishapSettings.SharpsDurationSeconds * 1000)
    BLScav_Sedated = false
    AnimpostfxStop("DrugsMichaelAliensFight")
    ResetPedMovementClipset(cache.ped, 0)
end

function ApplyVerminBite()
    local isCritter = math.random(1, 100) <= (Config.MishapSettings.CritterChance or 65) -- 65% chance to be a racoon
    if not isCritter and Framework.HasItem("rat_treats", 1) then
        TriggerServerEvent('bl_scav:server:UseRatTreats')
        Framework.Notify(Config.Lang['rat_treats'], "success", 5000)
    elseif not isCritter then
        local healthLoss = Config.MishapSettings.VerminHealthLoss 
        if not healthLoss then
            TraceLog("Error: healthLoss is not defined in Config")
            return
        end
        ...
        local newHealth = currentHealth - healthLoss
        newHealth = math.max(newHealth, 1) 
        Framework.LoadAnimDict(Config.VerminMishapAnim.dict)
        TaskPlayAnim(cache.ped, Config.VerminMishapAnim.dict, Config.VerminMishapAnim.anim, 8.0, 8.0, -1, 49, 0, false, false, false)
        ...
        local animTime = GetAnimDuration(Config.VerminMishapAnim.dict, Config.VerminMishapAnim.anim) * 1000
        if animTime < 2000 then
            animTime = math.random(2700, 4000)
        end
        Wait(animTime - 500)
    ...
    elseif isCritter then
        local healthLoss = Config.MishapSettings.CritterHealthLoss or 50
        SetEntityHealth(cache.ped, GetEntityHealth(cache.ped) - healthLoss)
        local currentHealth = GetEntityHealth(cache.ped)
        local newHealth = currentHealth - healthLoss
        newHealth = math.max(newHealth, 1) 
        ...
        ReleaseCritterCompanion(racoon)
...
RegisterNetEvent('bl_scav:snitchReport', function(coords, jobs)
...
RegisterNetEvent('bl_scav:client:DrinkBottleBadEffect', function()   -- effect when drinking hobo_bottle when not a hobo
    local health = GetEntityHealth(cache.ped)
    SetEntityHealth(cache.ped, health - 10)
end)

RegisterNetEvent('bl_scav:client:DrinkBottleGoodEffect', function()   -- effect when drinking hobo_bottle when you are a hobo
    local health = GetEntityHealth(cache.ped)
    SetEntityHealth(cache.ped, health + 10)
end)